...
const {rarity_analyze_normalized} = require(appRoot+"/cmd/rarity_analyze_normalized")
...
router.get("/:collectionName", function (req, res, next) {
  ...
  if (!_.isEmpty(search)) {
    ...
    totalPunkCountQuery =
      totalPunkCountQuery + ` WHERE ${req.params.collectionName}s.id LIKE :${req.params.collectionName}_id `;
    totalPunkCountQueryValue[`${req.params.collectionName}_id`] = "%" + search + "%";

    punksQuery = punksQuery + ` WHERE ${req.params.collectionName}s.id LIKE :${req.params.collectionName}_id `;
...
router.get("/:collectionName/matrix", function (req, res, next) {
  ...
  res.render("matrix", {
    ...
    ogDescription:
      config.collection_description + " | " + config.app_description,
    ogUrl: req.protocol + "://" + req.get("host") + req.originalUrl.replace("/matrix",""),        //ogUrl: req.protocol + "://" + req.get("host") + req.originalUrl,
    ogImage: config.main_og_image,
    activeTab: "matrix",
    allTraits: allTraits,
...
router.get("/wallet", function (req, res, next) {
  ...
  res.render("/:collectionName/wallet", {
