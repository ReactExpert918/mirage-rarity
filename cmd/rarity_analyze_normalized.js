...
exports.rarity_analyze_normalized = (configFile) => {
  ...
  let traitCountNum = db
    .prepare(`SELECT COUNT(*) as trait_count_num FROM ${collection}_trait_counts`)
    .get().trait_count_num;
  let traitCounts = db.prepare(`SELECT * FROM ${collection}_trait_counts`).all();
  let totalSupply = db
    .prepare(`SELECT COUNT(${collection}s.id) as ${collection}_total FROM ${collection}s`)
    .get().punk_total;
  let allTraits = db
    .prepare(
  ...
  let createScoreTableStmt =
    `CREATE TABLE normalized_${collection}_scores ( id INT, ${collection}_id INT, `;
  let insertPunkScoreStmt =
    `INSERT INTO normalized_${collection}_scores VALUES (:id, :${collection}_id, `;
  ...
  punkScores.forEach((punkScore) => {
    console.log(`Normalized ranking ${collection}: #` + punkScore.punk_id);
